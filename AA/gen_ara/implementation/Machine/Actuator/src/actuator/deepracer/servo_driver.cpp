...
namespace PWM {
    ...
    int ServoDriver::servoSubscriber(const float throttle, const float angle) {
    ...
    void ServoDriver::rawPWMSubscriber(const float throttle, const float angle) {
    ...
    int ServoDriver::setCalibrationValue(int cal_type, int min, int mid, int max, int polarity) {
    ...
    int ServoDriver::setGPIOActivation(bool enable) {
        bool ret = calibrateServo(GPIO_CHANNEL, enable);
        return (ret ? 0 : 1);
    }

    /// Handler for get calibration request
    int ServoDriver::getCalibrationValue(int cal_type, int* min, int *mid, int *max, int *polarity) {
        auto itCal = calibrationMap_.find(cal_type);
    ...
    int ServoDriver::writeCalJSON(const std::unordered_map<int, std::unordered_map<std::string, int>> &calibrationMap,
    ...
    int ServoDriver::setCalFromFile(std::unordered_map<int, std::unordered_map<std::string, int>> &calibrationMap,
                        const std::string &filePath) {
        Json::Value calJsonValue;
        Json::Reader reader;
    ...
    bool ServoDriver::calibrateServo(int pinNum, int pinVal) const {
