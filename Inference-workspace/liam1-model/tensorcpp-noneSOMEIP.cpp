...
#include <arpa/inet.h>
#include <unistd.h>// Sensor data structures
...
// Class for handling TCP/IP communication
class CommunicationHandler {
public:
    CommunicationHandler(const std::string& ip, int port) : server_ip_(ip), server_port_(port), sock_fd_(-1) {}
    ...
    void sendData(const std::vector<float>& data) {
        ...
        ssize_t bytes_sent = send(sock_fd_, data.data(), data_size, 0);
...
public:
    MainController(const std::string& modelPath, const std::string& ip, int port)
        : modelHandler_(modelPath), commHandler_(ip, port) {}
...
int main() {
    const std::string modelPath = "model.pb"; // Model PATH
    const std::string server_ip = "127.0.0.1"; // Server IP Address
    const int server_port = 12345; // Server port

    MainController controller(modelPath, server_ip, server_port);
    controller.run();
